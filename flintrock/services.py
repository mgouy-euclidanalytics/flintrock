...
class SparkNoteBook(FlintrockService):
    ...
    def install(
        ...
        try:
            with ssh_client.open_sftp() as sftp:
                sftp.put(
                    localpath=os.path.join(SCRIPTS_DIR, 'install-spark-notebook.sh'),
                    remotepath='/tmp/install-spark-notebook.sh')
                sftp.chmod(path='/tmp/install-spark-notebook.sh', mode=0o755)
            url = self.download_source
            ssh_check_output(
                client=ssh_client,
                command="""
                    set -e
                    /tmp/install-spark-notebook.sh {url}
                    rm -f /tmp/install-spark-notebook.sh
                """.format(url=shlex.quote(url)))

        except Exception as e:
            # TODO: This should be a more specific exception.
            print("Error: Failed to install SparkNoteBook.", file=sys.stderr)
            print(e, file=sys.stderr)
            raise
